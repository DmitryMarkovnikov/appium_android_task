...
$ANDROID_HOME/tools/bin/avdmanager --verbose create avd  -c 1000M --force --name test --device "Nexus 5X" --package "system-images;android-23;google_apis;x86" --tag "google_apis" --abi "x86"
if  [[ $1 = "-run" ]]; then
    $ANDROID_HOME/tools/emulator @test -gpu auto
fi

# usage example bootstrap.sh or bootstrap.sh -run